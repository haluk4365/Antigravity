...
- AI'ın referans kişiyi (anchor fotoğraftaki kişi) düzgün render etmesi kritik.
...
- Kapak fotoğrafında kullanıcının (anchor kişisi) görselden **kolayca ayırt edilebilmesi** zorunludur.
...
- **Hata örneği**: Tripo 3D 2 — Kapak 1 ve 2'de anchor kişisi yerine farklı kişi üretildi, Kapak 3'te doğru kişi üretildi.
...
### 21. Sporadik Yüz Drift — Vision Hard Gate ile Yakalanmalı 🔴 KRİTİK!
- Identity Lock + Master Anchor + PIXEL PRIORITY MODE prompt'u devredeyken bile, GPT Image 2 / Nano Banana sporadik olarak (genelde 6 kapağın 1-2'sinde) anchor kişisine benzeyen ama farklı bir kişi üretebilir. Bu **model bug'ı değil**, single-call variance — modeli değiştirmek yanlış teşhis.
- **Çözüm — vision review'da CRITICAL face gate**: `face_matches_reference: false` üretilen kapağın skorunu **anında 0'a düşürür** (Instant Fail kategorisi, text_present/duplicated ile aynı seviyede).
- Vision prompt'unda Face Identity Match kuralı **pixel-level** detaylandırılmalı: kemik yapısı, göz şekli + aralığı, burun köprüsü, dudak şekli, çene açısı, etnik kimlik, yaş. "Benzeyen ama farklı kişi → false". Tereddütte false'a yat — yanlış kapak ship etmektense retry tetiklemek daha ucuz.
- Hata örneği: Abacus 8 (12 May 2026) — 6 kapaktan 2'sinde (T1V1, T3V2) yabancı kişi üretildi. Vision review face_matches_reference field'ını okuyordu ama skoru sıfırlamıyordu — kapaklar Drive'a yine ship edildi. Hard gate eklendikten sonra aynı pipeline 5/6 doğru kapak + 1 skip üretti.

### 22. Tema 2 Caption — Script-Bound CURIOSITY Zorunlu! 🔴 KRİTİK!
- `generate_three_themes`'in Tema 2'si "CURIOSITY / MYSTERY angle" üretiyor. Eski jenerik instruction ("focus on mystery") Gemini'yi script'ten kopuk, içeriksiz hook'lara (BU NASIL MÜMKÜN?, SAKIN İZLEME, BUNA İNANAMAYACAKSIN, DURUN BİR DAKİKA, AKLINIZ DURACAK) yönlendiriyordu.
- **Yeni kural**: Tema 2 cover_text **MUTLAKA** script'teki somut detayı (sayı, fiyat, süre, araç adı, vaad) referans almalı. Counter-intuitive bir soru/iddia formunda olmalı ama jenerik değil.
- İyi örnekler (her biri scriptin somut detayına bağlı):
  - Script 3-ay-dev + no-code → "3 AY MI? 30 DK!"
  - Script 120K TL maliyet bypass → "120 BİN MI? SIFIR TL!" / "120 BİN MI? BEDAVA!"
  - Script işe alma vs AI → "MAAŞ MI? PROMPT MI?"
- Hata örneği: Abacus 8 ilk üretim — Tema 2 "BU NASIL MÜMKÜN?" (script Abacus + 120K TL + 3 ay süresinden hiçbir şey yok). Fix sonrası: "120 BİN MI? SIFIR TL!" — script'in 120.000 TL'sine doğrudan bağlı.

### 23. MIN_ACCEPTABLE_SCORE Guard — Retry Exhausted → SAVE ETME! 🔴 KRİTİK!
- Eski davranış: `run_autonomous_generation` retry tüketildiğinde `best_image_url`'ı her halükarda kaydediyordu — best_score 0 bile olsa. Yani retry'lar başarısız olsa bile **bozuk kapak Drive'a düşüyordu**.
- **Yeni kural**: `MIN_ACCEPTABLE_SCORE = 5` guard. Retry sonunda best_score bu eşiğin altındaysa save edilmez, `False` döner — o slot Drive'da boş kalır. Bu yan etkidir, kabul edilebilir; çünkü yanlış yüzlü/bozuk kapak ship etmek çok daha kötü.
- Hata örneği: Abacus 8 ilk üretim — T3V2 retry sonunda yüksek skor alamadığı halde "best" olarak kabul edildi, bozuk kapak Drive'a yüklendi. Fix sonrası: T3V2 iki denemede de score 3 aldı, skip edildi, Drive'a düşmedi.

## 📅 VERSİYON GEÇMİŞİ

### v7 — 12 Mayıs 2026
- Sporadik yüz drift'i için vision review face hard gate (Kural 21) — face_matches_reference=False → score=0
- Vision prompt'unda Face Identity Match → CRITICAL CHECK seviyesi, pixel-level detay (kemik yapısı, göz, burun, çene, etnik kimlik, yaş)
- Tema 2 CURIOSITY caption'ı zorunlu script-bound (Kural 22) — jenerik mystery hook listesi yasaklandı, scriptin somut detayına bağlama zorunluluğu
- MIN_ACCEPTABLE_SCORE=5 guard (Kural 23) — retry exhausted'da best_score<5 → save etme
- GPT Image 2 primary kaldı (Nano Banana 2 fallback); model swap yapılmadı, vakanın model değil variance olduğu doğrulandı
