...
Auth:
  Railway (prod): GMAIL_TOKEN_JSON env variable
  Lokal (dev): Merkezi google_auth modülü (_knowledge/credentials/oauth)
"""
import os
import sys
import json
...
# Özet mailinin gideceği + gönderen adres — .env dosyasından doldur.
NOTIFY_EMAIL = os.environ.get("NOTIFY_EMAIL", "")


# Merkezi OAuth modülündeki hesap anahtarı (.env'den override edilebilir).
GMAIL_ACCOUNT = os.environ.get("GMAIL_ACCOUNT", "account1")


def _get_gmail_service():
    """Gmail API service objesi döndür (mail gönderen hesap).

    Öncelik:
      1. Railway (prod): GMAIL_TOKEN_JSON env variable
    ...
    return get_gmail_service(GMAIL_ACCOUNT)
