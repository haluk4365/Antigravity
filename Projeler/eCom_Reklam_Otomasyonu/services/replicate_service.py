...
class ReplicateService:
    ...
    async def async_latentsync(
        ...
        """
        LatentSync (bytedance/latentsync) ile video üzerindeki dudak hareketlerini
        ses dosyasına göre tam senkronize eder.

        Args:
            video_url: Lip-sync yapılacak video dosyası URL'i (mp4)
            audio_url: Dudak hareketlerini yönlendirecek ses dosyası URL'i (mp3/wav)
            guidance_scale: Üretim guidance_scale parametresi
            seed: Sabit seed (tutarlılık için)

        Returns:
            str: Lip-synced video URL'i
        """
        import asyncio as _asyncio
        ...
        try:
            async with _asyncio.timeout(ASYNC_MERGE_HARD_TIMEOUT):
                log.info(
                    f"Async LatentSync lip-sync başlatılıyor: "
                    f"video_url={video_url[:80]}..., audio_url={audio_url[:80]}..."
                )
                ...
                log.info(f"Replicate LatentSync prediction oluşturuldu: {prediction.id}")
                ...
                for attempt in range(1, MAX_POLL_ATTEMPTS + 1):
                    ...
                    if prediction.status == "succeeded":
                        ...
                        if not output_url.startswith("http"):
                            raise RuntimeError(f"Replicate geçersiz output URL: {output_url[:100]}")
                        log.info(
                            f"LatentSync lip-sync tamamlandı: {prediction.id} "
                            f"({attempt} deneme)"
                        )
                        completed = True
                        return output_url
                    ...
                    if prediction.status == "failed":
                        completed = True
                        error = prediction.error or "Bilinmeyen hata"
                        log.error(f"LatentSync başarısız: {prediction.id} — {error}")
                        raise RuntimeError(f"LatentSync merge başarısız: {error}")

                    if prediction.status == "canceled":
                        completed = True
                        raise RuntimeError("LatentSync görev iptal edildi")

                    if prediction.status != prev_status:
                        log.info(
                            f"LatentSync polling [{attempt}/{MAX_POLL_ATTEMPTS}]: "
                            f"status {prev_status}→{prediction.status}"
                        )
                        prev_status = prediction.status
                    else:
                    ...
                    # LatentSync GPU üzerinde çalışır, adaptif polling
                    interval = 5 if attempt > 3 else 3
                    await _asyncio.sleep(interval)

                raise TimeoutError(
                    f"LatentSync timeout: {prediction.id} — "
                    f"{MAX_POLL_ATTEMPTS} deneme aşıldı"
                )
            ...
            if isinstance(e, TimeoutError):
                pid = getattr(prediction, "id", "?") if prediction else "?"
                log.error(
                    f"LatentSync timeout: hard_cap={ASYNC_MERGE_HARD_TIMEOUT}s, "
                    f"prediction={pid}"
                )
            raise
        except Exception:
            log.error("LatentSync async genel hatası", exc_info=True)
            raise
        finally:
            if prediction is not None and not completed:
                try:
                    await _asyncio.to_thread(prediction.cancel)
                    log.warning(f"LatentSync prediction iptal edildi (cleanup): {prediction.id}")
                except Exception:
                    log.warning(f"LatentSync prediction cancel basarisiz: {getattr(prediction, 'id', '?')}", exc_info=True)

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # 👄 WAV2LIP — Dil-Bagimsiz Lip Sync (Turkce icin tercih)
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # Model: devxpy/cog-wav2lip (audio-driven, language-agnostic)
    # LatentSync'e gore Turkce sesler icin daha iyi sonuc verir.

    WAV2LIP_VERSION = "8d65e3f4f4298520e079198b493c25adfc43c058ffec924f2aefc8010ed25eef"

    async def async_wav2lip(
        self,
        video_url: str,
        audio_url: str,
        pads: str = "0 10 0 0",
        ...
        """
        Wav2Lip (devxpy/cog-wav2lip) ile dudak senkronizasyonu.

        LatentSync'e alternatif — Turkce dahil her dilde calisir.
        Ses enerjisini dogrudan dudak hareketine esler, dil bagimsiz.

        Args:
            video_url: Lip-sync yapilacak video URL'i (mp4)
            audio_url: Ses dosyasi URL'i (mp3/wav)
            pads: Yuz padding "ust alt sol sag" (default: "0 10 0 0")
            resize_factor: Video kucultme carpani (1=orijinal, 2=yari)
            fps: Cikis fps (25 onerilen)
        ...
        try:
            async with _asyncio.timeout(ASYNC_MERGE_HARD_TIMEOUT):
                log.info(
                    f"Async Wav2Lip lip-sync baslatiliyor: "
                    f"video={video_url[:80]}..., audio={audio_url[:80]}..."
                )

                prediction = await _asyncio.to_thread(
                    self.client.predictions.create,
                    version=self.WAV2LIP_VERSION,
                    input={
                        "face": video_url,
                        "audio": audio_url,
                        "pads": pads,
                ...
                log.info(f"Wav2Lip prediction olusturuldu: {prediction.id}")
                ...
                for attempt in range(1, MAX_POLL_ATTEMPTS + 1):
                    try:
                        ...
                        reload_failures += 1
                        log.warning(f"Wav2Lip reload gecici hata ({reload_failures}/{MAX_RELOAD_FAILURES}): {reload_err}")
                        if reload_failures >= MAX_RELOAD_FAILURES:
                            raise RuntimeError(f"Wav2Lip reload tekrar tekrar basarisiz: {reload_err}")
                        await _asyncio.sleep(2)
                        continue
                    ...
                    if prediction.status == "succeeded":
                        output_url = prediction.output
                        if output_url is None:
                            raise RuntimeError(f"Wav2Lip succeeded ama output None: {prediction.id}")
                        if isinstance(output_url, list):
                            output_url = output_url[0] if output_url else None
                        output_url = str(output_url)
                        if not output_url.startswith("http"):
                            raise RuntimeError(f"Wav2Lip gecersiz output URL: {output_url[:100]}")
                        log.info(f"Wav2Lip lip-sync tamamlandi: {prediction.id} ({attempt} deneme)")
                        completed = True
                        return output_url
                    ...
                    if prediction.status == "failed":
                        completed = True
                        error = prediction.error or "Bilinmeyen hata"
                        log.error(f"Wav2Lip basarisiz: {prediction.id} — {error}")
                        raise RuntimeError(f"Wav2Lip basarisiz: {error}")

                    if prediction.status == "canceled":
                        completed = True
                        raise RuntimeError("Wav2Lip gorev iptal edildi")

                    if prediction.status != prev_status:
                        log.info(
                            f"Wav2Lip polling [{attempt}/{MAX_POLL_ATTEMPTS}]: "
                            f"status {prev_status} -> {prediction.status}"
                        )
                        prev_status = prediction.status
                    else:
                        log.debug(f"Wav2Lip polling [{attempt}/{MAX_POLL_ATTEMPTS}]: status={prediction.status}")

                    interval = 5 if attempt > 3 else 3
                    await _asyncio.sleep(interval)

                raise TimeoutError(f"Wav2Lip timeout: {prediction.id}")

        except (RuntimeError, TimeoutError) as e:
            if isinstance(e, TimeoutError):
                pid = getattr(prediction, "id", "?") if prediction else "?"
                log.error(f"Wav2Lip hard timeout ({ASYNC_MERGE_HARD_TIMEOUT}s): prediction={pid}")
            raise
        except Exception:
            log.error("Wav2Lip genel hatasi", exc_info=True)
            raise
        finally:
            if prediction is not None and not completed:
                try:
                    await _asyncio.to_thread(prediction.cancel)
                    log.warning(f"Wav2Lip prediction iptal edildi (cleanup): {prediction.id}")
                except Exception:
                    log.warning(f"Wav2Lip prediction cancel basarisiz: {getattr(prediction, 'id', '?')}", exc_info=True)
