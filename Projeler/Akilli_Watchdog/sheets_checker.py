    ...
    def full_check(self, project: dict) -> dict:
        ...
        for tab_name in tabs:
            ...
            if not tab_check["exists"]:
                error_msg = str(tab_check["error"])
                is_auth_error = any(kw in error_msg.lower() for kw in ["insufficient", "permission", "403", "forbidden", "unauthorized"])
                
                if is_auth_error:
                    tab_result["status"] = "AUTH_ERROR"
                    tab_result["error"] = tab_check["error"]
                    result["healthy"] = False
                    result["issues"].append(
                        f"⚠️ [{name}] Google Sheets API erişim hatası (403/Yetki yetersiz): {tab_check['error']}. "
                        f"Lütfen Google Sheets erişim izinlerini veya API token kapsamlarını kontrol edin."
                    )
                else:
