...
def run_health_check(force_email: bool = False) -> dict:
    ...
    # LLM analiz edilebilir pipeline tipleri (veri olan projeler)
    _llm_analyzable_pipelines = ("sheets_to_notion", "sheets_email", "custom_notion", "railway_only")

    if Config.GROQ_API_KEY:
        llm = LLMAnalyzer()
        for i, project in enumerate(Config.MONITORED_PROJECTS):
            # local_only ve static_site projeleri — analiz edilecek veri yok, atla
            if project.get("pipeline") not in _llm_analyzable_pipelines:
                logger.info(f"  ⏭️ [{project['name']}] LLM analizi atlandı ({project.get('pipeline', 'unknown')} pipeline)")
                continue
