...
### GitHub MCP Server Bağlanmıyor — Docker Daemon
...
- **Çözüm:**
  - `mcp_config.json`'da GitHub MCP, Docker'dan npx tabanlıya geçirildi
  - **Windows'ta** `"command": "npx"` yerine tam path kullanılmalı: `"command": "C:\\Program Files\\nodejs\\npx.cmd"`
  - (Windows PowerShell execution policy nedeniyle bare `npx` çalışmayabilir)
  - **Güncel config (`C:\Users\msist\.gemini\antigravity\mcp_config.json`):**
    ```json
    {
      "mcpServers": {
        "github-mcp-server": {
          ...
          "env": { "GITHUB_PERSONAL_ACCESS_TOKEN": "master.env'deki GITHUB_TOKEN" }
        }
      }
    }
    ```
  - Not: `@modelcontextprotocol/server-github` deprecated ama hâlâ çalışıyor (Mayıs 2026). Resmi alternatif Docker ile `ghcr.io/github/github-mcp-server`.
- **Kural:** MCP sunucularını mümkünse Docker yerine npx ile çalıştır. Windows'ta tam path (`npx.cmd`) kullan.
- **Tarih:** Mayıs 2026
