...
# ── HLK Admin Sohbet ────────────────────────────────────────────────────
HLK_ADMIN_SYSTEM = """Sen HLK'nin Yonetici Asistanisin. Yoneticiye HLK'nin nasil calistigini anlatirsin.

KURALLAR:
1. Yukaridaki VERILERI ve HLK URETIM SISTEMI bilgilerini kullan.
2. Yonetici "ajan siralamani goster" gibi bir sey sordugunda, HLK URETIM SISTEMI'ndeki
   ajan secim kriterlerini SIRALAYARAK acikla. "Mevcut degil" deme — metodolojiyi anlat.
3. Somut veri yoksa yontemi acikla, "sistemde mevcut degil" deme.
4. Kisa, net, en fazla 3 cumle.
5. Fiyatlar USD, TL karsiligi TCMB kuruyla."""
...
def _build_admin_context(user_data: dict, user_msg: str) -> str:
    ...
    return (
        ...
        f"AJAN SECIMI: Her video uretimi icin HLK, Research Orchestrator uzerinden dinamik ajan seçimi yapar.\n"
        f"  1. URUN SINIFLANDIRMA: Urunun kategorisi belirlenir (tekstil, kozmetik, elektronik, taki, vb.)\n"
        f"  2. AJAN HAVUZU: Kategoriye uygun tum ajanlar taranir — API anahtari, kota, erisim kontrol edilir.\n"
        f"  3. DEGERLENDIRME: Her ajan 9 kriter uzerinden puanlanir ve siralanir:\n"
        f"     - Urun Kategorisine Uygunluk (en onemli), Arastirma Kalitesi, Teknolojik Yeterlilik,\n"
        f"     - Dogruluk, Guvenilirlik, Hiz, Kaynak Cesitliligi, Guncellik, Maliyet.\n"
        f"  4. SIRALAMA: En yuksek puandan en dusuge siralanir. Esit puanda hiz ve maliyet ikincil kriterdir.\n"
        f"  5. GOREVLENDIRME: En yuksek puanli ajan gorevlendirilir. Basarisiz olursa siradaki devreye girer.\n"
        f"  6. OGRENME: Basarili ajan siralamasi 30 gun cache'lenir, benzer urunlerde yeniden kullanilir.\n"
        f"ARASTIRMA: Urun linki alinir → Link Validation → Research Orchestrator devreye girer:\n"
        f"  - Urun gorseli arastirmasi, Marka analizi, Urun aciklamalari\n"
        f"  - Hedef musteri analizi, Marka dili ve tarzi, Fiyat segmenti, Rakip analizi\n"
        f"  - Tum veriler birlestirilerek Reklam Stratejisi olusturulur (AR-002_13/35)\n"
        f"SENARYO: Arastirma ciktilari + hedef kitleye ozel tonlamayla senaryo uretilir.\n"
        f"  - 0-12 Cocuk: eglenceli/renkli | 13-17 Genc: dinamik/enerjik\n"
        f"  - 18-24 Genc Yetiskin: modern/trend | 25-34 Yetiskin: profesyonel/sik\n"
        f"  - 35-44 Aile: guvenilir/samimi | 45-54 Orta Yas: kaliteli/prestijli\n"
        f"  - 55+ Olgun: sakin/guven veren\n"
        f"  - Sahne sayisi: 4-6sn→2, 7-10sn→3, 11-20sn→4, 21+sn→5 sahne\n"
        f"  - Her sahnenin suresi video toplam suresine orantili dagitilir (~28/48/24%)\n"
        f"SES URETIMI: ElevenLabs TTS API kullanilir (AR-002_29/30/32)\n"
        f"  - Ses karakterleri: Kadin, Erkek, Cocuk (secime gore)\n"
        f"  - MASTER_REFERENCE_VOICE: Tonlama, enerji, ritim, vurgu standardi\n"
        f"  - Coklu dil: TR, EN, DE, FR, ES, AR, RU, KR — 8 dil destegi\n"
        f"  - TEXT_ONLY_MODE varsayilan; VOICE_ENABLED opsiyonel\n"
        f"VIDEO URETIMI: 10 adimli Production Runtime zinciri:\n"
        f"  1-4: On kosul dogrulama → 5: Runtime baslatma → 6: CEE PRE-CHECK\n"
        f"  7: PID olusturma (PID-YYYYMMDD-NNNN) → 8: Package olusturma\n"
        f"  9: Task paketi hazirlama → 10: Production Executor baslatma\n"
        f"  - CEE POST-CHECK: 6 boyutlu anayasal denetim (Kod-Anayasa, Flow, State, Operasyon, Mimari, Runtime)\n"
        f"  - Crash Recovery: PID user_data'ya kaydedilir, restart sonrasi kaldigi yerden devam\n"
        f"FIYATLANDIRMA:\n"
        f"  - Servis bedeli hesaplanir (API maliyetleri + ses + video uretimi)\n"
        f"  - Yonetici katsayisi ({katsayi}) uygulanir → Yonetici fiyati: ${yonetici_fiyat}\n"
        f"  - KDV (%20) eklenir → KDV dahil: ${kdvli}\n"
        f"  - TCMB kuru ({tcmb} TL) ile TL karsiligi: {tl_karsilik} TL\n"
        f"TEKNIK ALTYAPI:\n"
        f"  - Python 3.14 + python-telegram-bot + OpenAI GPT-4o\n"
        f"  - ElevenLabs TTS (ses) + Fal.ai Seedance (image-to-video)\n"
        f"  - Descript API (video duzenleme) + Hedra API (lip-sync)\n"
        f"  - Higgsfield AI (video uretimi) + Kie AI (gorsel uretimi)\n"
        f"  - Constitution Cache (SHA-256) + Constitution Index (1342 kural)\n"
        f"  - CEE (Constitution Enforcement Engine) + EEC (Execution Event Collector)\n"
        f"  - Olay Kayit Merkezi + LAC (Live Activity Center)\n"
        f"  - PID Runtime + Production Package Runtime + Production Executor\n\n"
        f"=== YONETICI SORUSU ===\n"
        f"{user_msg}"
    )
